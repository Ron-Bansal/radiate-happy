...
//         [data-hero-fade], [data-reveal], [data-project-card], [data-why-card] {
//       visibility: hidden; }
//       `,
//         }}
//       />
...
/* ═══════════════════════════════════════════════
   888 COUNTER (IntersectionObserver, no GSAP dependency)
   ═══════════════════════════════════════════════ */
...
const ChallengeIcon = () => (
  <svg viewBox="0 0 120 120" fill="none" className="w-full h-full">
    <circle cx="60" cy="60" r="44" stroke="#7F6DF1" strokeWidth="1.5" opacity="0.2" />
    <circle cx="60" cy="60" r="28" stroke="#7F6DF1" strokeWidth="1.5" opacity="0.35" />
    <circle cx="60" cy="60" r="12" fill="#7F6DF1" opacity="0.15" />
    <path d="M60 16V60L84 84" stroke="#7F6DF1" strokeWidth="1.5" strokeLinecap="round" />
  </svg>
);
const OutcomeIcon = () => (
  <svg viewBox="0 0 120 120" fill="none" className="w-full h-full">
    <rect x="20" y="70" width="16" height="30" rx="2" fill="#7F6DF1" opacity="0.12" />
    <rect x="44" y="50" width="16" height="50" rx="2" fill="#7F6DF1" opacity="0.2" />
    <rect x="68" y="30" width="16" height="70" rx="2" fill="#7F6DF1" opacity="0.3" />
    <path d="M28 65L52 45L76 25L100 15" stroke="#7F6DF1" strokeWidth="1.5" strokeLinecap="round" />
    <circle cx="100" cy="15" r="4" fill="#7F6DF1" opacity="0.5" />
  </svg>
);
const RangeIcon = () => (
  <svg viewBox="0 0 120 120" fill="none" className="w-full h-full">
    <circle cx="40" cy="45" r="18" stroke="#7F6DF1" strokeWidth="1.5" opacity="0.25" />
    <circle cx="80" cy="45" r="18" stroke="#7F6DF1" strokeWidth="1.5" opacity="0.25" />
    <circle cx="60" cy="78" r="18" stroke="#7F6DF1" strokeWidth="1.5" opacity="0.25" />
    <circle cx="60" cy="55" r="6" fill="#7F6DF1" opacity="0.18" />
  </svg>
);
...
/* ═══════════════════════════════════════════════
   HERO IMAGE WITH MOUSE PARALLAX LABELS
   ═══════════════════════════════════════════════ */
const heroLabels = [
  { text: "Product Builder", className: "absolute top-6 -left-4 z-20", depth: 0.03 },
  { text: "Studied CompSci, Finance & Business Analytics", className: "absolute top-16 -left-8 z-20", depth: 0.07 },
  { text: "Passionate about UX", className: "absolute top-24 -right-6 z-20", depth: 0.05 },
  { text: "Teach kids science + coding", className: "absolute top-[8.5rem] -right-1 z-20", depth: 0.035 },
  { text: "Huge dog person", className: "absolute bottom-4 -left-4 z-20", depth: 0.025 },
  { text: "Built this site to apply", className: "absolute bottom-4 left-[140px] z-20", depth: 0.045 },
];
...
const HeroImage = () => {
  ...
  useEffect(() => {
    if (!containerRef.current || !window.matchMedia("(pointer: fine)").matches) return;
    ...
    const loop = () => {
      ...
      labelRefs.current.forEach((label, i) => {
        if (!label) return;
        const d = heroLabels[i].depth;
        label.style.transform = `translate3d(${pos.current.x * d * 75}px, ${pos.current.y * d * 50}px, 0)`;
      });
...
const CarusoPage = () => {
  ...
  useLayoutEffect(() => {
    ...
    /* ── Lenis ── */
    const lenis = new Lenis({ duration: 1.2, wheelMultiplier: 0.85, touchMultiplier: 1.1, smoothWheel: true });
    lenis.on("scroll", ScrollTrigger.update);
    ...
    const onScroll = () => {
      ...
      else if (y > lastY + 5) nav.style.transform = "translateY(-100%)";
    ...
    const ctx = gsap.context(() => {
      ...
      /*
       * Philosophy:
       * - Content is ALWAYS visible (opacity: 1 by default in CSS)
       * - GSAP sets initial state to slightly faded (opacity 0.15, y 20)
       * - Then animates to full opacity
       * - If GSAP fails, content is fully visible anyway
       * - Triggers fire at 96% viewport height (almost immediately on enter)
       */

      // ── Hero: CSS animation handles the base, GSAP enhances ──
      const heroEls = gsap.utils.toArray<HTMLElement>("[data-hero]");
      if (heroEls.length) {
        gsap.set(heroEls, { opacity: 0.1, y: 18 });
        gsap.to(heroEls, {
          opacity: 1,
          y: 0,
          duration: 0.8,
      ...
      const scrollReveal = (selector: string, config?: {
        y?: number; duration?: number; stagger?: number;
        start?: string; trigger?: string;
      }) => {
        const { y = 20, duration = 0.7, stagger = 0, start = "top 96%", trigger } = config || {};
        const els = gsap.utils.toArray<HTMLElement>(selector);
        if (!els.length) return;

        gsap.set(els, { opacity: 0.15, y });

        const triggerEl = trigger ? root.querySelector(trigger) : undefined;

        if (triggerEl) {
          // Single trigger for a group (e.g. why cards)
          ScrollTrigger.create({
            trigger: triggerEl,
            start,
            once: true,
            onEnter: () => gsap.to(els, { opacity: 1, y: 0, duration, ease: "power2.out", stagger }),
          });
        } else {
      ...
      // Apply to sections
      scrollReveal("[data-fade]");
      scrollReveal("[data-project]", { y: 24, duration: 0.8 });
      scrollReveal("[data-why-card]", { y: 24, duration: 0.7, stagger: 0.06, trigger: "[data-why-section]", start: "top 92%" });
  ...
  /* ── WHY CARUSO DATA ── */
  const whyCards = [
    { icon: <ChallengeIcon />, title: "Challenging problem. Ambitious team", body: "Two years in, $50B+ in assets, 4x growth. That trajectory means the problems are real and the stakes are high. I've been looking for an environment alongside a highly ambitious, low-ego team. That's where I do my best work." },
    { icon: <OutcomeIcon />, title: "Outcome over output. AI as leverage", body: "Many companies have yet to fully realise the shift in PM roles in the age of AI. The vision Caruso has for this position excites me a lot as it describes how I naturally operate." },
    { icon: <RangeIcon />, title: "A role that rewards range", body: "I studied Computer Science, Finance, and Business Analytics at uni. Care deeply for design. Fluency with code. Enjoy speaking to customers and cross-collaborating. And love learning new things. Many PM roles ask you to pick a lane. This one seems to value someone who doesn't want to." },
  ];

  return (
    <div ref={pageRef} className="relative min-h-screen bg-[#132C38] font-['Open_Sans',sans-serif] text-[#515150]">
      ...
      <nav ref={navRef} className="fixed top-0 left-0 right-0 z-40 bg-[#132C38]/90 backdrop-blur-md border-b border-white/[0.06] transition-transform duration-300 ease-out">
        <div className="mx-auto flex w-[min(1280px,calc(100%-48px))] items-center justify-between py-3.5">
          ...
          <div className="flex items-center gap-5">
            <AucklandClock />
            <a href="mailto:raunaqbansal@outlook.com" className="rounded bg-[#7F6DF1] px-5 py-3 text-[12px] font-medium tracking-[0.06em] text-white transition-all duration-300 hover:bg-[#9D8DFF] hover:shadow-[0_4px_20px_rgba(127,109,241,0.3)]">
              Get in touch
            </a>
          </div>
        </div>
      ...
      <header className="relative min-h-screen bg-[#132C38] overflow-hidden pt-16">
        ...
        <div className="relative z-10 mx-auto grid min-h-[calc(100vh-64px)] w-[min(1280px,calc(100%-48px))] items-center gap-12 py-20 lg:grid-cols-[1.3fr_0.7fr]">
          <div>
            <h1 className="mb-2 font-['EB_Garamond',serif] text-[clamp(2.8rem,5.8vw,3.5rem)] font-normal leading-[1.15] tracking-[-0.015em] text-[hsl(60_3%_95%)]" data-hero>
              Caruso has the right idea.
            </h1>
            <div className="mb-12" data-hero>
              <h1 className="inline font-['EB_Garamond',serif] text-[clamp(2.8rem,5.8vw,3.5rem)] font-normal leading-[1.05] tracking-[-0.015em] text-[hsl(60_3%_95%)]">
                PMs should be{" "}
                <span className="relative inline-block">
                  creating
                  <span className="absolute bottom-0.5 left-[-3px] right-[-5px] h-[10px] rounded-[2px] bg-[#806EF1]" style={{ zIndex: -1 }} />
                </span>{" "}
                solutions.
              </h1>
            </div>
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2" data-hero>
          <div className="h-8 w-px bg-gradient-to-b from-white/20 to-transparent" />
        </div>
      </header>
      ...
      <section className="bg-[hsl(60_3%_95%)] py-24 lg:py-32">
        <div className="mx-auto w-[min(1280px,calc(100%-48px))]">
          ...
          <div className="grid gap-12 lg:grid-cols-[1.4fr_0.75fr] lg:gap-16 items-center min-h-[400px]">
            ...
            <div data-fade>
              <p className="mb-1 font-['EB_Garamond',serif] text-[clamp(3rem,5vw,4.2rem)] font-normal leading-[1] tracking-[-0.02em] text-[#132C38]">
                <AnimatedCounter target={888} />+
              </p>
              <p className="mb-8 text-[15px] font-medium tracking-[0.04em] text-[#152E39]">v0 prompts in 12 months</p>
              <div className="space-y-4 text-pretty">
                <p className="text-[15px] leading-[1.7] text-[#515150]">Over 25 prototypes, experiments, and solutions shipped. Always tinkering with new technology.</p>
                <p className="text-[15px] leading-[1.7] text-[#515150]">New product offerings. Internal tools. Customer-facing features.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto w-[min(1280px,calc(100%-48px))]">
          ...
          <div className="grid gap-6">
            {carusoProjects.map((project, index) => {
              const reverse = index % 2 !== 0;
              return (
                <article key={project.id} className="rounded-md bg-[hsl(60_3%_95%)] p-[clamp(22px,3vw,58px)] text-[#132C38]" data-project>
                  <p className="mb-2 text-[12px] font-semibold uppercase tracking-[0.2em] text-[#7F6DF1]">{project.category}</p>
                  <h3 className="mb-4 font-['EB_Garamond',serif] text-[clamp(1.7rem,3vw,2.8rem)] font-normal leading-[1.08] tracking-[-0.01em]">{project.title}</h3>
                  <div className="mb-12 flex flex-wrap gap-2">
                    {project.tags.map((tag) => (
                      <span key={tag} className="border border-[#7f6df180] rounded-[3px] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.15em] text-[#7F6DF1]">{tag}</span>
                    ))}
                  </div>
                  <div className="grid items-center gap-6 lg:grid-cols-[1.4fr_1fr]">
                    <div className={reverse ? "lg:order-2" : ""}>
                      <div className="overflow-hidden rounded shadow hover:shadow-md">
                        <img src={project.images[0]} alt={`${project.title} primary`} className="block aspect-[16/10] shadow w-full object-cover object-left-top transition duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] [filter:grayscale(10%)_contrast(1.02)] hover:scale-[1.03] hover:[filter:grayscale(0%)_contrast(1.05)]" loading="lazy" />
                      </div>
                    </div>
                    <div className={`border-l-2 border-[#938ED7] pl-4 ${reverse ? "lg:order-1" : ""}`}>
                      <p className="font-['EB_Garamond',serif] font-normal text-[clamp(1.4rem,2vw,1.85rem)] leading-[1.25]">{project.hook}</p>
                    </div>
                  </div>
                  <div className="mt-[34px] grid items-start gap-6 lg:mt-7 lg:grid-cols-[1.1fr_0.9fr_0.8fr]">
                    <div className={reverse ? "lg:order-2" : ""}>
                      <p className="mb-[22px] text-[14.5px] leading-[1.7] text-[#515150]">{project.body[0]}</p>
                      {project.body[1] && <p className="mb-[18px] text-[14.5px] leading-[1.7] text-[#515150]">{project.body[1]}</p>}
                    </div>
                    <div className={reverse ? "lg:order-1" : ""}>
                      <div className="overflow-hidden rounded shadow hover:shadow-md">
                        <img src={project.images[1]} alt={`${project.title} secondary`} className="block aspect-[4/3] w-full object-cover object-left-top transition duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] [filter:grayscale(10%)_contrast(1.02)] hover:scale-[1.03] hover:[filter:grayscale(0%)_contrast(1.05)]" loading="lazy" />
                      </div>
                    </div>
                    <div>
                      {project.body.slice(2).map((p) => (
                        <p key={p} className="mb-[18px] text-pretty max-w-[350px] text-[14px] leading-[1.8] text-[#515150]">{p}</p>
                      ))}
                    </div>
                  </div>
                </article>
      ...
      <section className="bg-white py-24 lg:py-32" data-why-section>
        <div className="mx-auto w-[min(1280px,calc(100%-48px))]">
          ...
          <div className="grid gap-4 mx-auto lg:grid-cols-3">
            {whyCards.map((card, i) => (
              <div key={i} className="group relative flex flex-col rounded-xl bg-[#F3F3F2] border border-white hover:border-[#273e481f] py-9 px-8 transition-all duration-500 hover:shadow-lg" data-why-card>
                <div className="flex h-[120px] w-[120px] scale-125 group-hover:scale-[1.4] mt-10 items-center justify-center self-center opacity-75 transition-all duration-500 group-hover:opacity-100">
                  {card.icon}
                </div>
                <div className="mt-12">
                  <h3 className="mb-3 font-['EB_Garamond',serif] text-[1.4rem] font-normal leading-[1.25] text-[#132C38]">{card.title}</h3>
                  <p className="text-[14px] mt-4 leading-[1.8] text-[#51514F]">{card.body}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mb-24 text-center" data-fade>
            ...
            <div className="mx-auto max-w-[920px] grid gap-6 lg:grid-cols-2">
              <div className="group relative overflow-hidden rounded-lg bg-[#F2F3F2] px-8 py-6 text-left transition-all duration-500 hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)]">
                ...
                <p className="relative font-['EB_Garamond',serif] text-[1.25rem] font-normal leading-[1.5] text-[#132C38]/80">Do fund managers in the US have different needs and behaviours to those from ANZ?</p>
              </div>
              <div className="group relative overflow-hidden rounded-lg bg-[#F2F3F2] px-8 py-6 text-left transition-all duration-500 hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)]">
                <span className="pointer-events-none absolute -top-8 -right-2.5 font-['EB_Garamond',serif] text-[7rem] font-normal leading-none text-[#7F6DF1]/[0.10] transition-colors duration-500 group-hover:text-[#7F6DF1]/[0.2]">02</span>
                <p className="relative font-['EB_Garamond',serif] text-[1.25rem] font-normal leading-[1.5] text-[#132C38]/80">Is there a space that borders fund administration where Caruso is deliberately choosing <strong>not</strong> to play?</p>
              </div>
            </div>
          </div>
          <div className="border-t border-white/[0.08] pt-7">
            <div className="flex flex-col items-center justify-between gap-5 sm:flex-row">
              <p className="text-[14px] tracking-[0.01em] text-[hsl(60_3%_95%/.85)]">Based in Auckland with NZ working rights</p>
              <div className="flex items-center gap-5">
                <a href="https://www.linkedin.com/in/ron-bansal/" target="_blank" rel="noopener noreferrer" className="group flex items-center gap-1.5 text-[14px] font-medium tracking-[0.03em] text-[hsl(60_3%_95%/.74)] transition-colors duration-300 hover:text-[#7F6DF1]"><LinkedInIcon />LinkedIn</a>
                <span className="h-3 w-px bg-white/[0.08]" />
                <a href="https://raunaqbansal.com" target="_blank" rel="noopener noreferrer" className="group flex items-center gap-1.5 text-[14px] font-medium tracking-[0.03em] text-[hsl(60_3%_95%/.74)] transition-colors duration-300 hover:text-[#7F6DF1]"><GlobeIcon />Portfolio</a>
                <span className="h-3 w-px bg-white/[0.08]" />
                <a href="mailto:raunaqbansal@outlook.com" className="group flex items-center gap-1.5 text-[14px] font-medium tracking-[0.03em] text-[hsl(60_3%_95%/.74)] transition-colors duration-300 hover:text-[#7F6DF1]"><MailIcon />Email</a>
              </div>
            </div>
          </div>
        </div>
